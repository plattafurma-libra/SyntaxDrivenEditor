...
(* JR from BB A character of a text has several attributes: 
font, color, and vertical offset. The font attribute consists of the 
font's typeface (e.g., Times), its size (e.g., 9 point), its weight (e.g., bold), 
and its style (e.g., italic, underlined, or strikeout). 
Vertical offsets such as superscripts or subscripts can be selected....,
...
TYPE  CONST (*scanner symbol classes*)
  ...
  TYPE Piece = POINTER TO PieceDesc;
	...
	Text* = POINTER TO RECORD
      ...
      changed*: BOOLEAN;
      notify*: Notifier;
      trailer: Piece (* points to sentinel in piece list *);
      pce: Piece;  (*cache*)
      org: LONGINT; (*cache*)
    END;
	...
	VAR t:Text;piece,sentinel,newPiece:Piece;i:INTEGER;
	
		PROCEDURE InsertPiece(newPiece,oldPiece:Piece);
		BEGIN
			newPiece.prev:=oldPiece;oldPiece.next:=newPiece;
	...
	BEGIN
		...
		FOR i:=0 TO 3 DO
			...
			InsertPiece(newPiece,piece);
			piece:=newPiece;
		END;
		sentinel.prev:=piece;
		TraversPieceTableForCheck(t);
		RETURN t;
	END Check;
